...
import { Loader2, Gift, Check, X, ClipboardCheck, Ban } from 'lucide-react'
...
import { Tabs as ContentTabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs'
import {
  Table,
  TableBody,
  TableCell,
...
import { approveRedemptionAction, rejectRedemptionAction, deliverRedemptionAction } from './actions'
...
export function Redemptions() {
  ...
  const [approveRedemptionId, setApproveRedemptionId] = useState<string | null>(null)
  ...
  const other = redemptions.filter((r) => r.status === 'REJECTED' || r.status === 'CANCELLED')
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold tracking-tight">Resgate de Prêmios</h1>
        <p className="text-muted-foreground text-sm font-light">
          Gerencie e controle as solicitações de troca de pontos por prêmios enviadas pelos alunos.
        </p>
      </div>
      ...
      <ContentTabs defaultValue="pending" className="w-full">
        ...
        <TabsContent value="pending" className="space-y-4">
          {pending.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border p-12 text-center">
              <Gift className="size-8 text-muted-foreground mx-auto mb-2 opacity-50" />
              <p className="text-sm text-muted-foreground">Nenhuma solicitação de resgate pendente.</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {pending.map((redemption) => (
                    <TableRow key={redemption.id}>
                      <TableCell className="font-medium">{redemption.student.name}</TableCell>
                      <TableCell>
                        {redemption.student.class.name} ({redemption.student.class.year})
                      </TableCell>
        ...
        <TabsContent value="approved" className="space-y-4">
          {approved.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border p-12 text-center">
              <Gift className="size-8 text-muted-foreground mx-auto mb-2 opacity-50" />
              <p className="text-sm text-muted-foreground">Nenhuma solicitação aprovada aguardando entrega.</p>
            </div>
          ) : (
            <div className="rounded-md border border-border overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {approved.map((redemption) => (
                    <TableRow key={redemption.id}>
                      ...
                      <TableCell>
                        {redemption.approvedAt ? new Date(redemption.approvedAt).toLocaleDateString('pt-BR') : '-'}
                      </TableCell>
        ...
        <TabsContent value="delivered" className="space-y-4">
          {delivered.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border p-12 text-center">
              <Gift className="size-8 text-muted-foreground mx-auto mb-2 opacity-50" />
              <p className="text-sm text-muted-foreground">Nenhuma entrega realizada.</p>
            </div>
          ) : (
            <div className="rounded-md border border-border overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {delivered.map((redemption) => (
                    <TableRow key={redemption.id}>
                      ...
                      <TableCell>
                        {redemption.deliveredAt ? new Date(redemption.deliveredAt).toLocaleDateString('pt-BR') : '-'}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <TabsContent value="other" className="space-y-4">
          {other.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border p-12 text-center">
              <Gift className="size-8 text-muted-foreground mx-auto mb-2 opacity-50" />
              <p className="text-sm text-muted-foreground">Nenhuma solicitação rejeitada ou cancelada.</p>
            </div>
          ) : (
            <div className="rounded-md border border-border overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {other.map((redemption) => (
                    <TableRow key={redemption.id}>
                      ...
                      <TableCell>
                        {new Date(
                          redemption.rejectedAt || redemption.cancelledAt || redemption.createdAt,
      ...
      {/* APPROVAL DIALOG */}
      <Dialog open={!!approveRedemptionId} onOpenChange={(open) => !open && setApproveRedemptionId(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Aprovar Solicitação de Resgate</DialogTitle>
            <DialogDescription>
              Você está aprovando o resgate do prêmio deste estudante. O saldo de pontos permanecerá consumido.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="cursor-pointer bg-emerald-500 hover:bg-emerald-600"
            >
              {isApprovePending ? <Loader2 className="size-4 animate-spin" /> : 'Confirmar Aprovação'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {/* REJECTION DIALOG */}
      <Dialog open={!!rejectRedemptionId} onOpenChange={(open) => !open && setRejectRedemptionId(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Rejeitar Solicitação de Resgate</DialogTitle>
            <DialogDescription>
              Você está rejeitando o resgate. Isso devolverá os pontos reservados imediatamente ao saldo do estudante.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-2 py-4">
            <Label htmlFor="rejection-reason">Justificativa da Rejeição (Obrigatório)</Label>
            <Input
              id="rejection-reason"
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="cursor-pointer"
            >
              {isRejectPending ? <Loader2 className="size-4 animate-spin" /> : 'Rejeitar Solicitação'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
